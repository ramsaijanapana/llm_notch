...
const CATALOG_AGENTS: ReadonlyArray<readonly [string, string]> = [
...
export const PREVIEW_AGENT_CATALOG: AgentCatalogEntry[] = CATALOG_AGENTS.map(
  ([id, displayName]) => {
    const verified = VERIFIED_CATALOG_IDS.has(id)
    return {
      id,
      displayName,
      aliases: [],
      executableNames: [],
      adapterFamily: verified ? 'nativeHooks' : 'undetermined',
      maturity: verified ? 'verifiedCurrent' : 'declaredUnverified',
      capabilities: verified
        ? [
            {
              capability: 'sessionEvents',
              availability: 'supported',
              quality: 'verifiedLocally',
              note: 'Covered by the current first-party integration.',
            },
          ]
        : [],
      configTargets: [],
