...
use llm_notch_desktop_lib::runtime::relay_path::{
    bundled_relay_filename, bundled_relay_in_resource_dir,
};
...
#[test]
fn bundled_relay_resource_path_uses_exe_suffix() {
    assert_eq!(bundled_relay_filename(), "llm-notch-relay.exe");
    let path = bundled_relay_in_resource_dir(std::path::Path::new("C:\\app\\resources"));
    assert!(path.ends_with("llm-notch-relay.exe"));
}
