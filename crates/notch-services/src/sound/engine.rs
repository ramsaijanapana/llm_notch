...
use super::{
    PlaybackOutcome, PlaybackRequest, SoundBackend, SoundBackendFactory, SoundError, SoundEvent,
    SoundRouting, SoundTheme, builtin_8_bit_theme,
};
...
impl SoundEngine {
    ...
    pub fn play_event(
        ...
        let asset = theme.events.get(&event).ok_or_else(|| {
            SoundError::Backend(format!("theme {theme_id} has no asset for {event:?}"))
        })?;
